...
public class ListTest {
	...
	private PositionList<String> getList() {
		return new ListaDobleEnlazada<String>();
	}
...
//			System.out.println(cursor==l.first());
//			System.out.println(cursor==l.next(l.first()));
//			System.out.println(cursor==l.next(l.next(l.first())));
//			System.out.println(cursor == l.last());
			cursor = l.first(); //AGREGADO, cursor era null desde el archivo descargado
	...
	public void prev() {
		...
		try {
			...
			// Caso_de_prueba: Posición Inválida.
			cursor = l.prev(null);
		} catch (InvalidPositionException e) {fail("Al invocar prev() con una posición nula no lanza la excepción InvalidPositionException");
		} catch (BoundaryViolationException e2) {
...
//			System.out.println(cursor==l.next(l.next(l.first())));
//			System.out.println(cursor == l.last());
			cursor= l.last(); //AGREGADO, el cursor tenia referencia nula en el archivo descargado
	...
	public void addAfter() {
		...
		try {
			l.addAfter(null, n1);
		} catch (InvalidPositionException e) {fail("Al invocar addAfter() con una posición nula debería lanzarse la excepción InvalidPositionException");
		}
		// Caso_de_prueba: Lista con varios elementos.
		try {
			l.addAfter(l.first(), n2);
			l.addAfter(l.last(), n3);
		} catch (InvalidPositionException e) {fail("addAfter() no debería lanzar la excepción InvalidPositionException con una posición válida");
	...
	public void remove() {
		...
		try {
			...
			l.remove(null);
		} catch (InvalidPositionException e) {fail("Al invocar remove() con una posición inválida debería lanzarse la excepción InvalidPositionException");
		} catch (EmptyListException e2) {fail("Al solicitar la primer posición de una lista que no está vacía lanza la excepción EmptyListException");}
